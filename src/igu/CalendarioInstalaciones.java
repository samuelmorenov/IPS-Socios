...
import logic.calendario_instalaciones.CalendarLogicInt;
...
public class CalendarioInstalaciones {

	private CalendarLogicInt cl;
	 ...
	 */
	public CalendarioInstalaciones() {
		initialize();
	}
		...
		if (btnBtnshowdate == null) {
			...
			btnBtnshowdate.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					cl.printFollowingWeek(day, month, year, (InstalacionDto) cbBoxSalas.getSelectedItem());
					refreshTable();
				}
			});
		}
	...
	private JTable addTablaFechas() {
		if(tableFechas == null) {
			String[] colName = { "Horas", "Lunes", "Martes", "Miercoles", "Jueves", "Viernes", "Sabado", "Domingo" };
			myData = new DefaultTableModel(colName, 0);
	...
	private JComboBox getCbBoxSalas() {
		if (cbBoxSalas == null) {
			...
			cbBoxSalas.setModel(new DefaultComboBoxModel(DatabaseLogic.getNombreSalas()));
		}
		return cbBoxSalas;
	}
